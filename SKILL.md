...
`kitty` or `tmux` + a graphical desktop but ignores the opt-in flag (it is an
explicit test). Each pane mocks a review by writing its own result file, so it exercises
...
- **If `PRR_FANOUT=true` (or the legacy `PRR_TMUX_FANOUT=true`) AND a graphical
  desktop is available AND `kitty` or `tmux` is on PATH** — on Linux a desktop
  means `DISPLAY` or `WAYLAND_DISPLAY` is set; on macOS (`uname` = `Darwin`) it
  always is — hand the whole batch to the fan-out launcher and do not review the
  PRs yourself. Run it **in the background** (`run_in_background: true`); it
  blocks until every review finishes, which is a long human-paced wait:

  ```
  ~/.claude/skills/prr/scripts/prr-fanout.sh <PR> <PR> [<PR> ...]
  ```

  Invoke it **bare** (no `PRR_FANOUT=true` prefix, no pipe) — the flag is
  ...
  It opens one window with one pane per PR (a kitty native grid when `kitty` is
  installed, otherwise tiled tmux panes in a terminal), each pane running `/prr`
  on a single PR with the approval gate fully intact. Each pane closes as that
  PR's review finishes (it watches the result file `post-review.sh` writes), then
  it prints a consolidated rollup. Do **not** run steps 1-6 for the batch
  yourself. When the background launcher exits, relay its rollup to the user.

- **Otherwise** (flag unset, no GUI, or neither kitty nor tmux) — fall back to
  reviewing the PRs **one at a time**: run the normal single-PR flow (steps 1-6)
  for the first PR, then the next, and so on. Never block on a missing
  GUI/terminal.
